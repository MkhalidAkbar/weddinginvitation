(function(){
  ...
  var LIBRARY=[
    ...
    {id:'aurora-editorial',name:'Aurora Editorial',style:'modern-editorial',paletteId:'aurora-sand',kind:'minimal',tags:['Modern','Minimalis','Editorial'],tagline:'Tata letak editorial modern \u2014 tipografi besar, section bernomor, galeri asimetris, garis waktu vertikal. Bentuk & posisinya benar-benar beda dari template floral.',mk:{bg1:'#fbfaf7',bg2:'#ecdfd4',ink:'#211d19',acc:'#b0894e',acc2:'#7d8a72'},theme:{cream:'#fbfaf7',ink:'#211d19',gold:'#b0894e',sage:'#7d8a72',blush:'#ecdfd4'}},
  ...
  function uploadMany(files){Array.prototype.slice.call(files||[]).forEach(function(f){ML_STATE.queue.push({file:f,name:f.name,status:'wait',progress:0})});runQueue();}
  ...
  function doUpload(p){ var inp=document.createElement('input'); inp.type='file'; inp.accept=(p==='music.src')?'audio/*':'image/*';
    inp.onchange=function(){ var file=inp.files[0]; if(!file) return;
      var path=userId+'/'+Date.now()+'_'+file.name.replace(/[^a-zA-Z0-9._-]/g,'_');
      edMsg('Mengunggah '+file.name+'…','ok');
      fetch(API+'/storage/v1/object/media/'+path,{method:'POST',headers:{apikey:KEY,Authorization:'Bearer '+token,'Content-Type':file.type||'application/octet-stream','x-upsert':'true'},body:file})
  ...
  p6Wire();
  function togglePublish(){if(cur.status==='published'){if(!confirm('Jadikan undangan kembali sebagai draft?'))return;db('sites?id=eq.'+cur.id,{method:'PATCH',headers:{Prefer:'return=minimal'},body:JSON.stringify({status:'draft',config:cfg})}).then(function(r){if(!r.ok)throw 0;cur.status='draft';updateStatusUI();edMsg('Undangan kembali menjadi draft.','ok')}).catch(function(){edMsg('Gagal mengubah status.','err')});return}p6Open()}
  ...
  function checkout(){
    ...
    return fetch('/.netlify/functions/create-payment',{method:'POST',headers:{'Content-Type':'application/json',Authorization:'Bearer '+token},
      body:JSON.stringify({site_id:cur.id,package:pkg,amount:amount,months:plan().months,customer:{name:userEmail,email:userEmail}})})
     .then(function(r){return r.json()})
  ...
  function openPreviewModal(){ if(!cur||!cfg) return; var m=el('previewModal'); if(!m) return; var pf=el('pvFrame');
    ...
    pf.onload=function(){ try{ var _pc=clone(cfg); _pc.noWatermark=paid; if(_pc.effects && !plan().fallingFx){ _pc.effects=Object.assign({},_pc.effects,{falling:''}); } var _send=function(){try{pf.contentWindow.postMessage({type:'WEDDING_PREVIEW',config:_pc},'*');}catch(e){}}; _send(); setTimeout(_send,350); setTimeout(_send,1400); }catch(e){} };
    pf.src=templateFile(cfg.style)+'?site='+encodeURIComponent(cur.slug)+'&preview=1';
    var pay=el('pvPay'); if(pay){var pl=plan();pay.textContent=pl.label;pay.setAttribute('data-package',currentPlan());}
    m.classList.add('on'); }
  function closePreviewModal(){ var m=el('previewModal'); if(m) m.classList.remove('on'); var pf=el('pvFrame'); if(pf) pf.src='about:blank'; }
