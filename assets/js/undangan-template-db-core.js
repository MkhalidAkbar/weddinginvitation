...
window.__applyWeddingConfig = function(c){
  ...
  /* TAHAP 2: urutan section + bangun ulang navdots */
...
window.__applyDeco=function(c){
  ...
  b.classList.toggle('orn-corners', O.corners===true);
  b.classList.toggle('orn-border', O.border===true);
  b.classList.toggle('orn-floral', O.floral===true);
  b.classList.toggle('orn-divider', O.divider===true);
  window.__buildFalling((c.effects&&c.effects.falling)||'');
};
window.__applyDeco(window.WEDDING_CONFIG||{});
/* FIX nempel: config Supabase dimuat async; panggil ulang setelah __configReady selesai supaya aksesoris/bingkai ikut tampil di undangan asli (deploy), bukan cuma di preview panel */
try{ Promise.resolve(window.__configReady).then(function(){ try{ window.__applyDeco(window.WEDDING_CONFIG||{}); }catch(e){} }); }catch(e){}
window.addEventListener('message',function(ev){
...
/* ====== Nav dots (Tahap 2: dibangun ulang mengikuti urutan & tampilan section) ====== */
window.__buildNav=function(){
  const nav=document.getElementById('navdots'); if(!nav) return;
  nav.innerHTML='';
  const secs=[...document.querySelectorAll('section')].filter(s=>getComputedStyle(s).display!=='none');
  secs.forEach((s)=>{const a=document.createElement('a');a.href='#'+(s.id||'');a.title=s.id;nav.appendChild(a)});
  const dots=[...nav.children];
  if(window.__navIO) window.__navIO.disconnect();
  window.__navIO=new IntersectionObserver((es)=>{es.forEach(e=>{if(e.isIntersecting){const i=secs.indexOf(e.target);dots.forEach(d=>d.classList.remove('active'));if(dots[i])dots[i].classList.add('active')}})},{threshold:.5});
  secs.forEach(s=>window.__navIO.observe(s));
};
window.__buildNav();
