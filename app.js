/* Priority 11 performance and accessibility runtime. */
(function(){
  if(document.querySelector('script[data-wedding-performance],script[src$="performance-a11y.js"]'))return;
  var current=document.currentScript,src=current&&current.src?current.src:'',base=src?src.slice(0,src.lastIndexOf('/')+1):'';
  var el=document.createElement('script');el.src=base+'performance-a11y.js';el.defer=true;el.setAttribute('data-wedding-performance','1');document.head.appendChild(el);
})();

/* Priority 8 analytics for every current/future template. */
(function(){
  if(document.querySelector('script[data-wedding-analytics],script[src$="analytics.js"]'))return;
  var current=document.currentScript,src=current&&current.src?current.src:'',base=src?src.slice(0,src.lastIndexOf('/')+1):'';
  var el=document.createElement('script');el.src=base+'analytics.js';el.defer=true;el.setAttribute('data-wedding-analytics','1');document.head.appendChild(el);
})();

/* Theme-aware form controls for every current/future template. */
(function(){
  if(document.querySelector('script[data-wedding-theme-ui],script[src$="theme-ui.js"]'))return;
  var current=document.currentScript,src=current&&current.src?current.src:'',base=src?src.slice(0,src.lastIndexOf('/')+1):'';
  var el=document.createElement('script');el.src=base+'theme-ui.js';el.defer=true;el.setAttribute('data-wedding-theme-ui','1');document.head.appendChild(el);
})();
...
window.__applyWeddingConfig = function(c){
  ...
  /* TAHAP 1: skala huruf, kepadatan bunga, tekstur kertas */
  var _bd=d.body, _ht=d.documentElement;
  if(_bd){ ['ts-xs','ts-sm','ts-lg','ts-xl'].forEach(function(k){_bd.classList.remove(k);}); if(c.textScale&&c.textScale!=='md') _bd.classList.add('ts-'+c.textScale);
    ['flora-off','flora-low','flora-high'].forEach(function(k){_bd.classList.remove(k);}); if(c.floraDensity&&c.floraDensity!=='med') _bd.classList.add('flora-'+c.floraDensity); }
  if(_ht){ ['grain-soft','grain-med','grain-strong'].forEach(function(k){_ht.classList.remove(k);}); if(c.grain&&c.grain!=='none') _ht.classList.add('grain-'+c.grain); }
  /* TAHAP 2: urutan section + bangun ulang navdots (jika template menyediakan) */
  if(c.sectionOrder && c.sectionOrder.length){ var _th=d.getElementById('thanks'); if(_th && _th.parentNode){ c.sectionOrder.forEach(function(id){ var _s=d.getElementById(id); if(_s && _s!==_th) _th.parentNode.insertBefore(_s,_th); }); } }
...
/* Gallery visibility safeguard */
(function(){function syncGalleryVisibility(){var c=window.cfg||window.CONFIG||window.config||{},S=c.sections||{};document.querySelectorAll('[data-section="gallery"],#gallery,.gallery-section,section.gallery').forEach(function(el){var cnt=el.querySelectorAll('img[src]:not([src=""]),[data-gallery-item]').length;var visible=S.gallery!==false&&cnt>0;el.hidden=!visible;el.style.display=visible?'':'none'})}if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',syncGalleryVisibility);else syncGalleryVisibility();new MutationObserver(syncGalleryVisibility).observe(document.documentElement,{subtree:true,childList:true,attributes:true,attributeFilter:['src']});window.syncGalleryVisibility=syncGalleryVisibility})();
